...
stages {
        ...
        stage('Docker run'){ // run container from our builded image
                steps {
                    ...
                    docker run -d --name nginx --link $DOCKER_ID/$DOCKER_IMAGE_CAST:$DOCKER_TAG --link $DOCKER_ID/$DOCKER_IMAGE_MOVIE:$DOCKER_TAG -v ./nginx_config.conf:/etc/nginx/conf.d/default.conf -p 8080:8080 nginx:latest
					sleep 10
                    '''
                    }
                }

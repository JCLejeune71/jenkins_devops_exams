...
stages {
        ...
        stage('Docker run'){ // run container from our builded image
                steps {
                    ...
                    docker run -d --name $DOCKER_IMAGE_MOVIE --link movie_db:movie_db -v ./movie-service:/app/ -p 8001:8000 -e DATABASE_URI=postgresql://movie_db_username:movie_db_password@movie_db/movie_db_dev -e CAST_SERVICE_HOST_URL=http://cast_service:8000/api/v1/casts/ $DOCKER_IMAGE_MOVIE:$DOCKER_TAG uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
                    ...
                    docker run -d --name $DOCKER_IMAGE_CAST --link cast_db:cast_db -v ./cast-service:/app/ -p 8002:8000 -e DATABASE_URI=postgresql://cast_db_username:cast_db_password@cast_db/cast_db_dev $DOCKER_IMAGE_CAST:$DOCKER_TAG uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
